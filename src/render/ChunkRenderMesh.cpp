...
bool ChunkRenderMesh::Upload(const MeshData& data, const Texture2D atlas) {
    Unload();
    if (data.Empty()) {
...
void ChunkRenderMesh::Unload() noexcept {
    if (ready_) {
        // The atlas is caller-owned; drop the reference so UnloadModel does not
        // free a texture that is still in use elsewhere.
